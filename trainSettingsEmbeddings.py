...
def getTrainingModels(lr=1e-4):
    ...
    models = get_transformer_models(patch_size, embed_dim, num_heads, hidden_dim, num_layers, dropout, use_pos_encoding=use_pos_encoding,tr_activation_fct=tr_activation_fct,name_suffix='_n', use_regression_token= use_regression_token, single_prediction=single_prediction)
    models_small = get_transformer_models(patch_size, embed_dim//2, num_heads//2, hidden_dim//2, num_layers//2, dropout, use_pos_encoding=use_pos_encoding,tr_activation_fct=tr_activation_fct,name_suffix='_s', use_regression_token= use_regression_token, single_prediction=single_prediction)
    models_big = get_transformer_models(patch_size, embed_dim*2, num_heads*2, hidden_dim*2, num_layers*2, dropout, use_pos_encoding=use_pos_encoding,tr_activation_fct=tr_activation_fct,name_suffix='_b', use_regression_token= use_regression_token, single_prediction=single_prediction)

    models.update(models_small)
...
def load_validation_data(length = 20):
    ...
    vid9 = trajectories_to_video(trajs9,nPosPerFrame,center=True,image_props=image_props)
    vid9,_ = normalize_images(vid9,background_mean,background_sigma,part_mean+background_mean)
    ...
    vid_in_order = vid_in_order.reshape(len(val_d_in_order),N_in_order,nFrames,patch_size,patch_size)
    vid_in_order,_ = normalize_images(vid_in_order,background_mean,background_sigma,part_mean+background_mean)

    return torch.Tensor(vid1),torch.Tensor(vid3), torch.Tensor(vid5), torch.Tensor(vid7), torch.Tensor(vid9), torch.Tensor(vid_in_order)
...
def get_transformer_models(patch_size = patch_size, embed_dim = embed_dim, num_heads = num_heads, hidden_dim = hidden_dim, num_layers = num_layers,
    ...
    # Define model instances
    models = {
        "linear" + name_suffix: GeneralTransformer(
        ...
        "cnn"+ name_suffix: GeneralTransformer(
        ...
        "deepcnn"+ name_suffix: GeneralTransformer(
