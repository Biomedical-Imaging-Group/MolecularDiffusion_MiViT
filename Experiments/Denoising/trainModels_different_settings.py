...
val_videos = load_validation_dataMult(RL_iterations,nFrames)  # Returns (vid1, vid3, vid5, vid7)
...
for cycle in range(num_cycles):
    ...
    for TrainingDs in TrainingDs_list:
        ...
        combined_videos  = trajs_to_vid_norm_rl(trajs,nPosPerFrame,center,image_props,RL_iterations)
        # Append to list
        all_videos.append(combined_videos)
    ...
    for name, model in models.items():
        ...
        with torch.no_grad():
            ...
            for vid, label_value in zip(val_videos, val_labels):
                ...
                # Multiply Labels by D_max to have values between 0 and 10 -> matches validations
                val_predictions = val_predictions * D_max_normalization

                val_loss = val_loss_function(val_predictions, label)
                # Optional: flatten the tensor if you want a 1D list
                val_loss_flat = val_loss.view(-1).tolist()  # list of float losses
                # If you still want an average to log globally:
                avg_val_loss = np.mean(val_loss_flat)
                ...
                label_losses.append(val_loss_flat)
